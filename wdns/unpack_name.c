		 ...
		 uint8_t *dst, size_t *sz)
{
	...
	while ((c = *src++) != 0) {
		if (c >= 192) {
			 ...
			 * referenced name starts before the start of the name fragment we are
			 * currently unpacking, which catches compression pointer loops.
			 */
			if (cptr >= start)
